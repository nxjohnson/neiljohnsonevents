...
- [ ] Confirm Weddings / Events / Headshots galleries are set to **public**
- [ ] Find each album's `AlbumKey` and fill in `src/config/galleries.ts`
- [ ] Set `SMUGMUG_API_KEY` and `SMUGMUG_NICKNAME` in `.env` (copy from `.env.example`)
- [ ] Set the same two as Cloudflare Pages **build** environment variables once deployed

## Content
- [ ] Replace placeholder bio in `src/pages/about.astro` with real copy
- [ ] Double check homepage/testimonial copy in `src/pages/index.astro` matches what you want live
